...
//Searching:
   ...
   for (i=list_min; i<=list_max; i+=1) begin
    ...
    if i=list_max {
     NSP_notify("SCRIPT: nsp_statement_positions. ERROR: (1) Syntax error ("+argument2+" statement).");
   ...
   for (i=pos_1+1; i<=list_max; i+=1) begin
    ...
    if i=list_max {
     NSP_notify("SCRIPT: nsp_statement_positions. ERROR: (2) Syntax error ("+argument2+" statement).");
     return nspToken[NSP_TOK.abort];
     }
   end;
   ...
   for (i=pos_2+1; i<=list_max; i+=1) begin
    ...
    if i=list_max and pos_3=-1 {
     NSP_notify("SCRIPT: nsp_statement_positions. ERROR: (3) Syntax error ("+argument2+" statement).");
     return nspToken[NSP_TOK.abort];
     }  
   end;
