...
if nsp_is_equal(av,nspToken[NSP_TOK.abort]) {
 NSP_notify("SCRIPT: nsp_execute_single. ERROR: Aborting because evaluation failed.");
...
if as_var_max<>-1 {
 ...
 switch (nspListPar[|as_var_max]) begin
    ...
    if (list_min=as_var_max-1 and nspListStr[|list_min]=nspToken[NSP_TOK.dsm_name]) {
     mode=3;
      ...
      if nsp_is_equal(target,nspToken[NSP_TOK.abort]) {
       NSP_notify("SCRIPT: nsp_execute_single. ERROR: Cannot assign value because target could not be evaluated.");
       exit;
       }
  ...
  default:
    NSP_notify("SCRIPT: nsp_execute_single. ERROR: Cannot assign value - illegal target.");
    exit;
   break;  
